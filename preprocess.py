...
parser.add_argument('--output_dir', default='data/preprocessed_binary', help="Where to write the new dataset")
...
def modify_columns_and_concat(df_detect, df_undetect, skymaps, skymapnames):
    ...
    df_undetect['pixel'] = hp.ang2pix(nside=4096, theta=df_undetect['true_dec'].apply(lambda dec: np.deg2rad(90 - dec)), phi = df_undetect['true_ra'].apply(lambda ra: np.deg2rad(ra)))
    df_undetect = df_undetect[['pixel','detected','true_g','true_r','true_i','true_z']]
    df_undetect[['true_g','true_r','true_i','true_z']] = df_undetect[['true_g','true_r','true_i','true_z']].apply(lambda m: 10**((30-m)/2.5))

    df_detect['pixel'] = hp.ang2pix(nside=4096, theta=df_detect['unsheared/dec'].apply(lambda dec: np.deg2rad(90 - dec)), phi = df_detect['unsheared/ra'].apply(lambda ra: np.deg2rad(ra)))
    df_detect['detected'] = np.ones(len(df_detect))
    df_detect[['true_g','true_r','true_i','true_z']] = df_detect[['BDF_FLUX_DERED_G','BDF_FLUX_DERED_R','BDF_FLUX_DERED_I','BDF_FLUX_DERED_Z']]#.apply(lambda x: -2.5*np.log10(x)+30)
...
def split_and_normalize(df, output_dir):
    ...
    train_df[[str(col) for col in train_df.columns][2:]] = (train_df[[str(col) for col in train_df.columns][2:]] - train_mean) / train_std
    test_df[[str(col) for col in test_df.columns][2:]] = (test_df[[str(col) for col in test_df.columns][2:]] - train_mean) / train_std
    val_df[[str(col) for col in val_df.columns][2:]] = (val_df[[str(col) for col in val_df.columns][2:]] - train_mean) / train_std
    pd.DataFrame({'train_mean': train_mean, 'train_std': train_std}, index=[str(col) for col in train_df.columns][2:]).to_pickle(os.path.join(output_dir,'normalization.pkl'))
...
def preprocess(args):
    ...
    for dslice in ['test', 'train', 'val']:
        ...
        true = torch.from_numpy(split_df[dslice][['true_g','true_r','true_i','true_z']].values).float().contiguous()
