...
parser.add_argument('--restore_file', default=None, help=\
                    "Optional, name of the file in --test_dir \
...
def train_step(model, optimizer, loss_fn, conditions, true, out):
    ...
    optimizer.zero_grad()
    z = Variable(torch.randn(conditions.shape[0],\
                             1)).requires_grad_(True).cuda(non_blocking=True)
...
def train(model, optimizer, dataloader, loss_fn, acc, params):
    ...
    with tqdm(total=len(dataloader)) as t:
        for i, (conditions_batch, true_batch, \
                out_batch) in enumerate(dataloader):
            
            conditions_batch, true_batch, out_batch = Variable(conditions_batch), \
                Variable(true_batch), Variable(out_batch)
            
            if params.cuda:
                conditions_batch, true_batch, out_batch = conditions_batch.cuda(non_blocking=True), \
                    true_batch.cuda(non_blocking=True), out_batch.cuda(non_blocking=True)
...
def evaluate(model, dataloader, loss_fn, acc, params):
    ...
    for conditions_batch, truth_batch, out_batch in dataloader:
        ...
        if params.cuda:
            conditions_batch, truth_batch, out_batch, z_batch = conditions_batch.cuda(non_blocking=True), \
